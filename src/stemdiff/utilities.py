...
class PolarProcessor:
    """
    Polar Gaussian reconstruction method for diffraction patterns.

    This processor enhances diffraction peaks by transforming the image to polar coordinates,
    fitting row-wise Gaussian mixtures along angular profiles, and reconstructing a denoised
    peak-focused image back in Cartesian space.

    Unlike direct blob detectors, this method models signal structure in polar space:
    each radius row is treated as a 1D signal over angle, where peaks are detected, filtered,
    and approximated by Gaussian components.

    The pipeline is:

    1) **Input preparation + optional centering helper**
       The class provides `_center_image()` to center an image by center-of-mass (COM)
       using affine translation with zero padding.
       In current `process_image()` flow, the input image is used directly as `centered_image`
       (no automatic shift is applied there), but `_center_image()` is available for upstream use.

    2) **Background suppression**
       A light rolling-ball correction (`bcorr.rolling_ball(..., radius=1)`) is applied before
       polar conversion to reduce smooth background and improve peak contrast.

    3) **Cartesian -> polar transform**
       `_cartesian_to_polar()` samples the input image on a polar grid:
         - angles: 0..360 degrees with step `angle_step`
         - radii: 0..`radius`
       Interpolation is done via `scipy.ndimage.map_coordinates` (linear, constant fill outside image).

    4) **Row-wise peak modeling in polar space**
       For each polar row (starting from row index 15):
         - robust local noise estimate is computed (MAD-like)
         - adaptive prominence threshold is scaled by row index (`radius_factor`)
         - `scipy.signal.find_peaks` proposes candidates
         - candidates are filtered by prominence/width/height-vs-background criteria
         - a constrained multi-Gaussian model is fit via `scipy.optimize.curve_fit`
           using `multi_gaussian_numba` as the model function
         - if fit fails, row returns zeros (safe fallback)

       Output per row is a smooth resampled 1D signal plus fitted peak parameters
       `(amplitude, mean, std)`.

    5) **Polar reconstruction**
       Fitted rows are inserted into a full polar buffer (`full_resampled_polar`),
       preserving original polar image shape.

    6) **Polar -> Cartesian reconstruction**
       `_polar_to_cartesian()` remaps reconstructed polar data back to Cartesian coordinates
       with interpolation (linear, constant outside support).

    7) **Diagnostics / visualization**
       If `show_plots=True`, `_plot_results()` shows:
         - original image
         - reconstructed image
         - original polar transform
         - resampled polar Gaussian approximation
       It also prints summary stats:
         - average peaks per processed row
         - total number of fitted peaks

    Parameters
    ----------
    None in constructor.

    Primary runtime parameters are passed to methods:

    `process_image(image, show_plots=False)`
        image : np.ndarray
            2D diffraction image (recommended float-compatible grayscale).
        show_plots : bool, default=False
            If True, display processing diagnostics.

    `_cartesian_to_polar(image, center=None, radius=None, angle_step=1)`
        center : tuple[int, int] or None
            Polar origin `(x, y)`. Defaults to geometric image center.
        radius : int or None
            Max sampled radius. Defaults to distance from origin to image corner proxy.
        angle_step : float
            Angular step in degrees.

    `_polar_to_cartesian(polar_image, output_shape, center=None, angle_step=1)`
        output_shape : tuple[int, int]
            Output Cartesian shape `(H, W)`.

    Returns
    -------
    process_image(...) -> np.ndarray
        Reconstructed Cartesian image (same shape as input), where angular peak structure
        has been denoised/regularized through row-wise Gaussian fitting in polar space.

    Internal row processor output:
    `_process_single_row(...) -> (row_index, resampled_data, peak_params)`
        row_index : int
        resampled_data : np.ndarray
            Reconstructed row signal over angle.
        peak_params : list[tuple[float, float, float]]
            Fitted `(amplitude, mean, std)` triplets.

    Notes
    -----
    - The method is very slow and not really recomended
"""
...
class DiffractionPeakSegmenter:
    """
    Polar-threshold diffraction peak segmentation with centroid concentration.

    This class extracts diffraction peaks by thresholding in polar space, mapping the mask back
    to Cartesian coordinates, and collapsing each detected blob into a single Dirac-like pixel
    at its centroid (standard or power-weighted CoM). The output is a sparse intensity map, same as before

    The pipeline is:

    1) **Validate + initialize**
       Constructor validates numerical ranges (rows, thresholds, radii, angle step, blob size),
       stores processing parameters, and prepares internal state (`center`, polar dimensions).

    2) **Center stabilization (COM shift)**
       `_calculate_shift_and_center()`:
         - computes geometric center
         - estimates center-of-mass only inside `center_radius` disk where center peak should be
         - shifts image so estimated COM aligns with geometric center
       This improves radial consistency before polar conversion.

    3) **Optional pre-mask of bright center**
       In `process_image()`, if `pre_mask_center_pixels > 0`, a circular region around center
       is zeroed before polar thresholding to suppress direct-beam / center glow artifacts.

    4) **Cartesian -> polar conversion**
       `_to_polar()` samples the centered image on a polar grid:
         - radius dimension = `polar_radius` (or auto from image size)
         - angle dimension = `np.arange(0, 360, angle_step)`
       Sampling is linear interpolation with constant zero outside support.

    5) **Adaptive row-wise threshold in polar domain**
       `_adaptive_threshold_polar()` processes rows from `start_row` outward:
         - computes row median + MAD
         - threshold = median + `threshold_factor` * robust_sigma
           (`1.4826 * MAD`, fallback to std when MAD ~ 0)
         - marks pixels above threshold
       Result is a binary polar mask focused on ring/arc signal away from center.

    6) **Polar mask -> Cartesian mask**
       `_to_cartesian()` remaps the binary polar mask back to Cartesian space.
       Mask is binarized with `> 0.5` and optionally blob-filtered by area
       (`min_blob_size`, connected components).

    7) **Blob concentration to Dirac-like peaks**
       `_find_peaks_and_concentrate()`:
         - connected-components labeling of Cartesian mask
         - per-blob intensity sum from centered image (`ndimage.sum_labels`)
         - centroid computation:
             * standard CoM if `centroid_power == 1`
             * power-weighted CoM otherwise (weights = intensity^p)
         - writes each blob’s summed intensity to one rounded centroid pixel
           (accumulates if multiple blobs collide after rounding)

    8) **Shift back to original coordinates**
       `_shift_back()` applies inverse shift (nearest-neighbor) so output aligns with
       original image coordinate frame.

    9) **Optional visualization**
       `show_results=True` renders a diagnostic panel:
         - original image
         - centered image
         - polar mask
         - Cartesian mask(s)
    ...
    Parameters
    ----------
    start_row : int, default=55
        First polar radius row included in adaptive thresholding.
        Rows below this are ignored to avoid center artifacts.

    threshold_factor : float, default=3
        Multiplier for robust row threshold (MAD-based).

    center_radius : int, default=100
        Radius of disk used for initial COM estimation and centering.

    pre_mask_center_pixels : int, default=0
        Radius of center suppression mask applied before polar thresholding.
        Set > 0 to suppress central hotspot leakage into mask.

    angle_step : float, default=1.0
        Angular sampling step in degrees for polar transforms.

    polar_radius : Optional[int], default=None
        Number of radial samples in polar space.
        If None, derived automatically from image size.

    min_blob_size : int, default=30
        Minimum connected-component area (pixels) kept in Cartesian mask.
        Set 0 to disable blob-size filtering.

    centroid_power : float, default=2
        Centroid weighting exponent:
          - 1.0 => standard center of mass
          - >1 => emphasizes brighter pixels in centroid placement

    verbose : bool, default=False
        Enables detailed logs and intermediate stats.

    Method Parameters
    -----------------
    process_image(image: np.ndarray, show_results: bool = False) -> np.ndarray
        image : np.ndarray
            2D diffraction frame.
        show_results : bool
            If True, plots detailed diagnostics.

    Returns
    -------
    np.ndarray
        2D float concentrated peak image (same shape as input).
        Each retained blob contributes one non-zero pixel at centroid location,
        with value equal to summed blob intensity in centered image.

    Notes
    -----
    - This is one of two best methods we found, together with blob detectors below. Use this.
    """
...
class DiffractionPeakFinder:
    """
    Classical diffraction peak finder with configurable background subtraction,
    multi-detector support, optional detector voting, and Dirac-delta output.

    This class processes a 2D diffraction image, suppresses background, detects peak
    candidates with one of several detector families, integrates peak intensities
    via circular apertures, and returns a sparse image where each peak is represented
    by a single pixel (Dirac-like map) holding integrated intensity.

    Supported detectors
    -------------------
    - 'log'   : Laplacian of Gaussian blobs
    - 'doh'   : Determinant of Hessian blobs
    - 'mser'  : Maximally Stable Extremal Regions (OpenCV)
    - 'pcbr'  : Principal-curvature-based response (Hessian eigenvalue + local maxima)
    - 'vote'  : consensus over ['log', 'doh', 'mser', 'pcbr']
    ...
    -------------------
    1) **Input checks + cast**
       `process_image()` validates that input is a 2D NumPy array and converts to float64.

    2) **Background estimation**
       `_estimate_background()` dispatches to selected method:
       - opening/opening_downsampled: morphological opening with disk element
       - gaussian: large-sigma smoothing
       - median: median filter with disk footprint
       - rolling_ball: rolling-ball baseline estimate
       - polynomial: radial polynomial fit to low-intensity percentile pixels

    3) **Background subtraction + clipping**
       Subtracted image = `image - background`, then clipped to non-negative range.

    4) **Noise estimation**
       `_estimate_noise()` computes robust sigma using MAD (normal scale factor),
       with safe fallback if MAD is zero/NaN.

    5) **Peak detection**
       `_detect_peaks()` calls one detector runner:
       - `_run_log()`: optional downsample, blob_log, scale coordinates back
       - `_run_doh()`: optional downsample, blob_doh, scale coordinates back
       - `_run_mser()`: region detection on uint8-normalized image, region centroids
       - `_run_pcbr()`: Hessian -> eigenvalues -> response map -> local maxima
       - `_run_vote()`: runs multiple detectors, groups nearby points, keeps consensus
       Then common post-processing removes detections inside
       `center_ignore_radius_pixels` around image center.

    6) **Integrated intensity (aperture sum)**
       For each surviving detection, intensity is integrated from background-subtracted
       image in a circular aperture:
       - LoG/DoH: radius ≈ `integration_sigma_factor * sigma_score`
       - Other detectors: fixed `integration_fixed_radius`
       Intensities are summed inside the aperture mask (local ROI for efficiency).

    7) **Dirac-delta image construction**
       Peaks are stored as `(row, col, integrated_intensity, score)`, sorted by intensity.
       Output image is zeros; each peak contributes intensity to one rounded pixel.
       Overlaps are accumulated using `np.add.at`.

    8) **Optional plotting**
       `_plot_results()` visualizes:
       - original image
       - estimated background
       - background-subtracted image with peak markers
       - optional PCBR response map
       - final Dirac image (LogNorm when safe, otherwise linear normalization)

    9) **Result caching**
       Intermediate and final artifacts are stored in `last_*` attributes for debugging
       and downstream analysis.

    Parameters
    ----------
    detector_method : str, default='log'
        Peak detector. One of `SUPPORTED_DETECTORS`.

    background_method : str, default='opening_downsampled'
        Background estimator. One of `SUPPORTED_BACKGROUNDS`.

    threshold_factor : float, default=3.5
        Global detection threshold scale (used with estimated noise sigma).

    center_ignore_radius_pixels : int, default=50
        Peaks inside this center radius are discarded.

    Background parameters
    ---------------------
    bg_disk_radius : int, default=70
        Structuring-element radius for morphological opening.

    bg_gaussian_sigma : float, default=20.0
        Sigma for Gaussian background estimate.

    bg_median_radius : int, default=20
        Disk radius for median-filter background.

    bg_rolling_ball_radius : int, default=30
        Radius for rolling-ball background estimate.

    bg_poly_degree : int, default=4
        Polynomial degree for radial polynomial background fit.

    bg_poly_percentile : float, default=25
        Percentile threshold used to select low-intensity points for poly fit.
        Must be in (0, 100).

    LoG / DoH parameters
    --------------------
    peak_min_sigma : float, default=6
    peak_max_sigma : float, default=60
    num_sigma_steps : int, default=30
    log_scale : bool, default=False
        Detector configuration for scale-space blob detection.

    log_doh_downsample_factor : int, default=4
        Optional speed-up for LoG/DoH by processing downsampled image and
        mapping detections back.

    MSER parameters
    ---------------
    mser_delta : int, default=5
    mser_min_area : int, default=60
    mser_max_area : int, default=14400

    PCBR parameters
    ---------------
    pcbr_sigma : float, default=3.0
        Hessian scale.

    pcbr_lambda_thresh : float, default=0.5
        Negative-eigenvalue threshold for ridge-like response.

    pcbr_response_thresh_rel : float, default=0.1
        Relative threshold for local maxima extraction in response map.

    pcbr_min_distance : int, default=5
        Minimum spacing between PCBR local maxima.

    Voting parameters
    -----------------
    vote_radius : float, default=5.0
    ...
    ----------------------
    integration_sigma_factor : float, default=3
        Aperture radius multiplier for LoG/DoH (sigma-based radius).

    integration_fixed_radius : int, default=5
        Fixed aperture radius for MSER/PCBR/vote detections.

    verbose : bool, default=False
        Enables detailed runtime/debug logging.

    Returns
    -------
    process_image(image, show_result=False) -> np.ndarray
        2D float Dirac image (same shape as input):
        each detected peak is a single non-zero pixel equal to integrated intensity
        after background subtraction.

    Stored outputs (debug/state)
    ----------------------------
    last_peaks_data : list[(row, col, intensity, score)]
        Final detected peaks sorted by intensity (descending).

    last_noise_sigma : float
        MAD-based sigma from subtracted image.

    last_background_img : np.ndarray
        Estimated background.

    last_subtracted_img : np.ndarray
        Non-negative background-subtracted image.

    last_dirac_img : np.ndarray
        Final sparse Dirac output.

    last_detector_map : np.ndarray or None
        Detector-specific map (used by PCBR response visualization).

    last_vote_details : dict or None
        Per-method detections when using `detector_method='vote'`.

    Notes
    -----
    - The output is intentionally sparse and suited for downstream peak statistics,
      matching, or tracking rather than dense segmentation.
    - Detection quality strongly depends on the chosen background model and thresholding.
    - DoH threshold scaling may require dataset-specific tuning.
    - Voting is often more robust to detector-specific failure modes but slower.
    - If multiple peaks round to the same pixel, their intensities are summed.
    """
    ...
    def process_image(self, image, show_result=False):
        ...
        try:
            ...
            if self.last_peaks_data:
                ...
                peak_intensities_all = np.array([p[2] for p in self.last_peaks_data])
                ...
                if self.verbose:
                    ...
                    if np.any(np.isnan(peak_intensities_all)):
                        ...
                        peak_intensities_all = np.nan_to_num(peak_intensities_all)
